...
class Member {
    ...
    public function changeLevel($faction, $steamid, $newRank) {
        $query = Database::getFactory()->getConnection(DB_NAME)->prepare("UPDATE members SET mainlevel = :rank WHERE faction = :faction AND steamid = :steamid LIMIT 1");
        $query->execute(array(
            ':rank' => $newRank,
            ':faction' => $faction,
            ':steamid' => $steamid
    ...
    public function getLastSeen($steamid, $value = "lastcopseen") {
        $thisPlayer = Database::getFactory(true)->getConnection(DB_NAME_LIFE, array(DB_HOST_LIFE, DB_USER_LIFE, DB_PASS_LIFE))->prepare(
            "SELECT playerid, ".$value." FROM PHXClients WHERE playerid = :steamid LIMIT 1"
        );
